...
def build_plan():
    ...
    def links(text, kind=''):
        ...
        for word in ['战吼','亡语','回响','启咒','耀眼','影蔽','造物','放逐','眩晕','立即行动','指示物','伤害分担','任务','预见','每日','永久','金币','护甲','基础数值','基础生命值','伤势']:
    ...
    tq={
      '护':'哨位在后排／法术堆时是否也对前排提供战吼护甲待确认。','破':'哨位是否受沉默／控制及其目标选择细节待确认。','锐':'哨位耗尽、免死牺牲、战后清理是否均触发亡语，增益持续范围待确认。','咒':'获得法术进入本场队列还是收藏、耐久及出现范围待确认。',
      ...
      '小费':'法术被消耗指本场触发还是战后耐久归零移出待确认。','法术傀儡':'类型已确认为随从；战吼／亡语法术的选择流程及其触发边界待确认。','奥能融合':'两张或三张的条件、产物品级与耐久待确认。','天眼':'自动预见消耗／奖励排除范围与事件已见计数待确认。','拾荒':'恢复英雄护盾已替代旧回血；随机纹章池已确认，奖励次数及预见判定待确认。',
    }
...
def finish_supplement():
    ...
    for r in before:
        ...
        if name=='太阳':
            ...
            fields={'规范描述':r['fields']['原始描述'],'审阅状态':'初版待确认','待确认事项':'已补全为造物随从。战吼与亡语法术的选择池、选择时机，以及是否消耗所选法术耐久仍待确认。'}
        else: continue
        for key in fields:
