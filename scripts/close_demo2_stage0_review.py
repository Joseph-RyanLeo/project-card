...
'爆弹学徒':(None,'遗愿在小队死亡结算、移除并腾出位置后触发，在原小队右侧召唤1张诡雷；不在消灭前触发。',None),
...
'持盾新兵':('突击：相邻远程友军获得保护1。','突击：使相邻、当前远程行动的友军获得保护1；不再监听相邻关系变化自动发放。',None),
...
'重装骑士':('突击：获得等同于护甲的强化。','突击结算时读取当前护甲，获得等量强化；不是随护甲实时变化的基础数值。',None),
...
'辎重驮夫':(None,'突击获得1金币；该次突击结算时若乡邻生效，额外获得2金币。不是全体生命加成。',None),
'标枪散兵':('突击：以数值+2进行1次远程行动，随后变为近战。','突击立即进行1次数值+2的远程行动，正常触发对应行动效果，随后锁定近战；不是2倍倍率。',None),
...
'集结号':(None,'突击：相邻友军本场行动冷却×0.8；不再为所有友军增加基础数值。',None),
...
'民兵指挥官':('友方人类每有一个相邻人类，获得+1/+1。','逐个计算友方人类自身的相邻人类数量，每个提供数值与生命+1；不再统计全场相邻组合或使用最多3组上限。','这是实时邻接加成，还是突击时固定本场加成？'),
'外交官':('友方随从每有一个不同种族的相邻友军，获得+1/+1。','按每个友方随从自身的异族相邻友军数量获得数值与生命加成；取消嵌套乡邻。','是否保留原突击触发，还是改为随相邻变化实时生效？'),
...
for r in snap[card]:
 ...
 replacements={'随军啤酒机':['回响','相邻','充能Ns'],'持盾新兵':['突击','相邻','远程','保护N'],'重装骑士':['突击','强化N','护甲'],'胆怯的步兵':['友军行动','强化N','次数上限'],'并肩作战':['乡邻','固定减伤'],'民兵指挥官':['相邻人类','基础数值','基础生命值'],'外交官':['相邻','其他种族','基础数值','基础生命值'],'集结号':['装备','突击','相邻','行动冷却缩短X%']}
