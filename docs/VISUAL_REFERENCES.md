...
此图只作为整体布局参考，不作为运行时背景。敌我人物必须同时存在：敌方人物沿用旧素材且固定正面；我方人物使用用户提供的像素动图透明图集，准备阶段的我方战场＋收藏视图显示正面，敌我战场视图显示背面。点击我方人物切换视角时播放 8 帧转身动作，返回时倒序播放，动作时长与视角移动同为 0.32 秒。
